...
function renderSteps(v, b) {
  ...
  const steps = eligible ? [
    ...
    ['כנס היכרות בנושא פרישה', 'לרוב מתקיים כנס/הרצאה לעובדים המועמדים לפרישה (כולל בני/בנות זוג) — סוקר את המסלולים, היבטי המס והטעויות הנפוצות.'],
    ['פגישה אישית + סימולציית טופס 161', 'עוד לפני סיום העבודה בפועל — בדיקת התמונה הפיננסית, הפנסיונית והמיסויית האישית שלכם.'],
    ['החלטה תוך 21 יום', 'לאחר קבלת הסימולציה האישית מטעם ההנהלה — יש 21 יום להודיע על הרצון לפרוש (פרישה מרצון).'],
    ['ליווי עד השלמת התהליך', 'קרן תבחן כדאיות, תסביר את המשמעות המיסויית והפנסיונית, ותלווה מול המעסיק, חברת הביטוח ורשות המסים עד לסיום בפועל.'],
  ] : [
